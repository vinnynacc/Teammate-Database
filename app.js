...
(async function () {
  ...
  // Accepts either encoded or plain filenames and returns a single, correct encoding
  function safeFile(file) {
    if (!file) return 'placeholder.png';
    try {
      return encodeURIComponent(decodeURIComponent(file));
    } catch (e) {
      // If decode fails (malformed %), just encode whatever we got
  ...
  function buildCard(p) {
    ...
    const photo = BASE + safeFile(p.photoFile);
